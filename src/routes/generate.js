...
router.post("/", async (req, res) => {
    try {
        ...
        const baseName = `GL - ${staffName} - ${todayStamp}`;
...
router.post("/import", upload.single("file"), (req, res) => {
    try {
        ...
        const importedPayload = extractGeneratedPayload(req.file.buffer);
        if (!importedPayload) {
            return res.status(400).json({
                message: "This DOCX does not contain import metadata. Use a file generated by this system."
            });
        }

        return res.status(200).json({
            message: "Template imported successfully.",
            values: importedPayload
        });
    } catch (error) {
        return res.status(400).json({
